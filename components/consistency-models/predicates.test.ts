...
import { PRAM, RVal, SequentiallyConsistent, SingleOrder } from './predicates'
import {
...
describe('sequential consistency', () => {
    ...
    // test('the thing i was failing', () => {
    //     const h = generateOperationHistoryFromString(`
    //         ------[A:x<-3]--------------[C:x->4]
    //         -----------------[B:x->3]-----------
    //     `)

    //     const foo = generateSerialization(h, 'A B C')

    //     expect(PRAM(h, [foo, foo])).toBe(true)
    // })
})
